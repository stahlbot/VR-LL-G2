...
bool SRanipalLip_Core::GetLipWeightings_Name(TMap<FName, float>& shapes)
{
	bool valid = UpdateData();
	if (valid)
	{
		...
		case SupportedLipVersion::version1:
			shapes.Empty();
			for (auto shape : Weightings)
			{
				auto kkk = shape.Key;
				const UEnum* EnumClass = FindObject<UEnum>(ANY_PACKAGE, TEXT("LipShapeEnum"), true);
				if (EnumClass)
				{
					FText displayValue = EnumClass->GetDisplayNameTextByValue((int64)shape.Key);
					FName keyName = FName(*displayValue.ToString());
					shapes.Add(keyName, shape.Value);
				}
			}
			break;
		case SupportedLipVersion::version2:
			shapes.Empty();
			for (auto shape : Weightings_v2)
			{
				const UEnum* EnumClass = FindObject<UEnum>(ANY_PACKAGE, TEXT("LipShapeEnum_v2"), true);
				if (EnumClass)
				{
					FText displayValue = EnumClass->GetDisplayNameTextByValue((int64)shape.Key);
					FName keyName = FName(*displayValue.ToString());
